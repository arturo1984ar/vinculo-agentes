...
function getHTML() {
  return [
    ...
    'function loadBusquedas(){notionQ(BUSQ_DB,{property:"Estado",select:{equals:"Abierta"}}).then(function(d){busquedas=(d.results||[]).map(function(p){return{id:p.id,puesto:(p.properties&&p.properties.Puesto&&p.properties.Puesto.title&&p.properties.Puesto.title[0])?p.properties.Puesto.title[0].plain_text:"Sin titulo",cliente:(p.properties&&p.properties.Cliente&&p.properties.Cliente.rich_text&&p.properties.Cliente.rich_text[0])?p.properties.Cliente.rich_text[0].plain_text:"",perfil:(p.properties&&p.properties["Perfil Ideal"]&&p.properties["Perfil Ideal"].rich_text&&p.properties["Perfil Ideal"].rich_text[0])?p.properties["Perfil Ideal"].rich_text[0].plain_text:"",sector:(p.properties&&p.properties.Sector&&p.properties.Sector.rich_text&&p.properties.Sector.rich_text[0])?p.properties.Sector.rich_text[0].plain_text:"",seniority:(p.properties&&p.properties.Seniority&&p.properties.Seniority.select)?p.properties.Seniority.select.name:""};});var sel=document.getElementById("sel-busqueda");sel.innerHTML="<option value=\\"\\">Seleccion\u00e1 una b\u00fasqueda...</option>"+busquedas.map(function(b){return "<option value=\\""+b.id+"\\">"+b.puesto+(b.cliente?" \u2014 "+b.cliente:"")+"</option>";}).join("");var badge=document.getElementById("header-badge");badge.textContent=busquedas.length+" b\u00fasqueda"+(busquedas.length!==1?"s":"")+" activa"+(busquedas.length!==1?"s":"");badge.className="badge "+(busquedas.length>0?"badge-green":"badge-gray");}).catch(function(){document.getElementById("header-badge").textContent="Error de conexi\u00f3n";document.getElementById("header-badge").className="badge badge-red";});}',
    ...
    'function generarOutreach(){var nombre=document.getElementById("c-nombre").value.trim();var perfil=document.getElementById("c-perfil").value.trim();var busqueda=document.getElementById("c-busqueda").value.trim();if(!nombre||!perfil||!busqueda)return;document.getElementById("btn-outreach").disabled=true;document.getElementById("outreach-step").style.display="inline";document.getElementById("resultado-outreach").innerHTML="";callClaude(OUTREACH_SYS,"Candidato: "+nombre+"\\nPerfil: "+perfil+"\\nB\u00fasqueda: "+busqueda).then(function(raw){var r=JSON.parse(raw.replace(/```json|```/g,"").trim());document.getElementById("resultado-outreach").innerHTML="<div class=\'card\'><div style=\'display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;\'><div class=\'card-title\' style=\'margin-bottom:0;\'>Email</div><button class=\'btn btn-sm btn-ghost\' onclick=\'copiarTexto(\\"email-body\\",this)\'>Copiar</button></div><div style=\'font-size:12px;color:var(--text3);margin-bottom:8px;\'>Asunto: <strong>"+r.asunto+"</strong></div><div id=\'email-body\' class=\'copy-block\'>"+r.mensaje+"</div></div><div class=\'card\'><div style=\'display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;\'><div class=\'card-title\' style=\'margin-bottom:0;\'>LinkedIn</div><button class=\'btn btn-sm btn-ghost\' onclick=\'copiarTexto(\\"li-body\\",this)\'>Copiar</button></div><div id=\'li-body\' class=\'copy-block\'>"+r.mensajeLinkedIn+"</div></div>";}).catch(function(e){console.error(e);}).finally(function(){document.getElementById("btn-outreach").disabled=false;document.getElementById("outreach-step").style.display="none";});}',
    'function copiarTexto(id,btn){navigator.clipboard&&navigator.clipboard.writeText(document.getElementById(id).innerText);btn.textContent="\u2713 Copiado";setTimeout(function(){btn.textContent="Copiar";},2000);}',
    'function cargarSeguimiento(){var btn=document.getElementById("btn-seg");btn.disabled=true;btn.textContent="Cargando...";notionQ(PROC_DB,{property:"Estado",select:{equals:"Contactado"}},[{property:"Fecha de Contacto",direction:"ascending"}]).then(function(d){var items=(d.results||[]).map(function(p){return{id:p.id,nombre:(p.properties&&p.properties.Candidato&&p.properties.Candidato.title&&p.properties.Candidato.title[0])?p.properties.Candidato.title[0].plain_text:"Sin nombre",busqueda:(p.properties&&p.properties["B\u00fasqueda Asociada"]&&p.properties["B\u00fasqueda Asociada"].rich_text&&p.properties["B\u00fasqueda Asociada"].rich_text[0])?p.properties["B\u00fasqueda Asociada"].rich_text[0].plain_text:"",fecha:(p.properties&&p.properties["Fecha de Contacto"]&&p.properties["Fecha de Contacto"].date)?p.properties["Fecha de Contacto"].date.start:null};});var lista=document.getElementById("lista-seguimiento");if(!items.length){lista.innerHTML="<div style=\'text-align:center;color:var(--text3);font-size:13px;padding:40px;\'>No hay candidatos contactados por ahora</div>";}else{lista.innerHTML=items.map(function(p){var dias=p.fecha?Math.floor((Date.now()-new Date(p.fecha))/86400000):null;var urgente=dias!==null&&dias>5;return "<div class=\'seg-card\' style=\'border-left:3px solid "+(urgente?"#DC2626":"#52B788")+";\' ><div style=\'display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;\'><div><div style=\'font-weight:500;font-size:14px;\'>"+p.nombre+"</div><div style=\'font-size:12px;color:var(--text3);margin-top:2px;\'>"+p.busqueda+"</div></div>"+(dias!==null?"<span class=\'badge "+(urgente?"badge-red":"badge-blue")+"\'>"+dias+" d\u00edas sin respuesta</span>":"")+"</div><div id=\'fu-"+p.id+"\'><button class=\'btn btn-sm btn-ghost\' onclick=\'genFollowup(\\""+p.id+"\\",\\""+p.nombre.replace(/"/g,"")+"\\",\\""+p.busqueda.replace(/"/g,"")+"\\","+(dias||0)+")\'>Generar follow-up</button></div></div>";}).join("");}}).catch(function(){document.getElementById("lista-seguimiento").innerHTML="<div style=\'color:#DC2626;padding:16px;\'>Error cargando</div>";}).finally(function(){btn.disabled=false;btn.textContent="Actualizar";});}',
    'function genFollowup(id,nombre,busqueda,dias){var div=document.getElementById("fu-"+id);div.innerHTML="<span style=\'font-size:12px;color:var(--text3);\'>Generando...</span>";callClaude("Agente de recruiting. Gener\u00e1s follow-ups breves y directos. Solo el texto, sin JSON.","Candidato: "+nombre+". B\u00fasqueda: "+busqueda+". D\u00edas sin respuesta: "+dias+".").then(function(txt){div.innerHTML="<div class=\'result-box\'>"+txt+"</div>";}).catch(function(){div.innerHTML="<span style=\'color:#DC2626;font-size:12px;\'>Error generando</span>";});}',
    'loadBusquedas();',
    '<\/script>',
    '</body>',
    '</html>'
