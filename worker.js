...
function handlePDF(input) {
  ...
  area.innerHTML = "<div style='font-size:24px'>&#9203;</div><div>Leyendo PDF...</div>";
  ...
  reader.onload = function(e) {
    pdfjsLib.getDocument({ data: e.target.result }).promise.then(function(pdf) {
      ...
      Promise.all(pages).then(function(txts) {
        document.getElementById("txt-perfil").value = txts.join("\\n");
        area.innerHTML = "<div style='font-size:24px'>&#9989;</div><div>" + file.name + "</div>";
      });
    });
  };
...
document.getElementById("upload-area").addEventListener("drop", function(e) {
  ...
  if (file && file.type === "application/pdf") {
    var inp = document.getElementById("pdf-input");
...
function notionQ(dbId, filter, sorts) {
  var body = {};
  if (filter) body.filter = filter;
  if (sorts) body.sorts = sorts;
  return fetch("/functions/notion", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ path: "/v1/databases/" + dbId + "/query", method: "POST", body: body })
  }).then(function(r) { return r.json(); });
}

function notionCreate(dbId, props) {
  return fetch("/functions/notion", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ path: "/v1/pages", method: "POST", body: { parent: { database_id: dbId }, properties: props } })
  }).then(function(r) { return r.json(); });
}

function callClaude(sys, user) {
  return fetch("/functions/claude", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
function loadBusquedas() {
  notionQ(BUSQ_DB, { property: "Estado", select: { equals: "Abierta" } }).then(function(d) {
    busquedas = (d.results || []).map(function(p) {
      var props = p.properties || {};
      return {
        id: p.id,
        puesto: props.Puesto && props.Puesto.title && props.Puesto.title[0] ? props.Puesto.title[0].plain_text : "Sin titulo",
        cliente: props.Cliente && props.Cliente.rich_text && props.Cliente.rich_text[0] ? props.Cliente.rich_text[0].plain_text : "",
        perfil: props["Perfil Ideal"] && props["Perfil Ideal"].rich_text && props["Perfil Ideal"].rich_text[0] ? props["Perfil Ideal"].rich_text[0].plain_text : "",
        sector: props.Sector && props.Sector.rich_text && props.Sector.rich_text[0] ? props.Sector.rich_text[0].plain_text : "",
        seniority: props.Seniority && props.Seniority.select ? props.Seniority.select.name : ""
      };
    });
    var sel = document.getElementById("sel-busqueda");
    sel.innerHTML = "<option value=''>Seleccion\u00e1 una b\u00fasqueda...</option>" + busquedas.map(function(b) {
      return "<option value='" + b.id + "'>" + b.puesto + (b.cliente ? " \u2014 " + b.cliente : "") + "</option>";
    }).join("");
...
function onBusquedaChange() {
  var b = busquedas.find(function(x) { return x.id === document.getElementById("sel-busqueda").value; });
  var info = document.getElementById("busq-info");
...
function procesarCandidato() {
  var texto = document.getElementById("txt-perfil").value.trim();
  var busqId = document.getElementById("sel-busqueda").value;
  if (!texto || !busqId) { setError("Carg\u00e1 un CV y selecci\u00f3n una b\u00fasqueda."); return; }
  var busq = busquedas.find(function(b) { return b.id === busqId; });
  ...
  callClaude(PARSER_SYS, "Extr\u00e9 la informaci\u00f3n de este perfil:\\n\\n" + texto.slice(0, 8000)).then(function(raw1) {
    var c = JSON.parse(raw1.replace(/\`\`\`json|\`\`\`/g, "").trim());
    window._c = c;
    var rp = document.getElementById("resultado-parser");
    rp.style.display = "block";
    var campos = [["Nombre", c.nombre], ["Cargo", c.cargoActual], ["Empresa", c.empresaActual], ["Experiencia", c.anosExperiencia ? c.anosExperiencia + " a\u00f1os" : null], ["Sector", c.sector], ["Ubicaci\u00f3n", c.ubicacion], ["Email", c.email]].filter(function(x) { return x[1]; });
    ...
    return callClaude(SCORER_SYS, "CANDIDATO:\\n" + JSON.stringify(c) + "\\n\\nBUSQUEDA:\\nPuesto: " + busq.puesto + "\\nCliente: " + busq.cliente + "\\nSector: " + busq.sector + "\\nSeniority: " + busq.seniority + "\\nPerfil: " + busq.perfil);
  }).then(function(raw2) {
    var s = JSON.parse(raw2.replace(/\`\`\`json|\`\`\`/g, "").trim());
    window._s = s;
    var recColor = { Avanzar: "badge-green", Evaluar: "badge-yellow", Descartar: "badge-red" };
    var scoreBg = s.score >= 75 ? "#D8F3DC" : s.score >= 55 ? "#FFF3CD" : "#FEE2E2";
    var scoreFg = s.score >= 75 ? "#1B4332" : s.score >= 55 ? "#5C3D00" : "#7F1D1D";
    var dimLabels = { fitDeRol: "Fit de rol", fitDeSeniority: "Seniority", fitDeIndustria: "Industria", fitDeEmpresa: "Tipo empresa" };
    function barColor(v) { return v >= 75 ? "#52B788" : v >= 55 ? "#FBBF24" : "#F87171"; }
    var rs = document.getElementById("resultado-scorer");
    rs.style.display = "block";
    var dims = Object.keys(s.dimensiones || {}).map(function(k) {
      var v = s.dimensiones[k];
      return "<div><div style='display:flex;justify-content:space-between;font-size:12px;'><span style='color:var(--text3);'>" + (dimLabels[k] || k) + "</span><span style='font-weight:500;'>" + v + "</span></div><div class='bar-bg'><div class='bar-fill' style='width:" + v + "%;background:" + barColor(v) + ";'></div></div></div>";
    }).join("");
    var forts = s.fortalezas && s.fortalezas.length ? "<div><div style='font-size:11px;font-weight:500;color:#16A34A;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;'>Fortalezas</div>" + s.fortalezas.map(function(f) { return "<div style='font-size:12px;color:var(--text2);margin-bottom:3px;'>\u2022 " + f + "</div>"; }).join("") + "</div>" : "";
    var brechs = s.brechas && s.brechas.length ? "<div><div style='font-size:11px;font-weight:500;color:#DC2626;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;'>Brechas</div>" + s.brechas.map(function(b) { return "<div style='font-size:12px;color:var(--text2);margin-bottom:3px;'>\u2022 " + b + "</div>"; }).join("") + "</div>" : "";
    ...
    setError("Error procesando. Intent\u00e1 de nuevo.");
    setStep("");
    console.error(e);
  }).finally(function() {
...
function guardarNotion() {
  var c = window._c; var s = window._s;
  var busqId = document.getElementById("sel-busqueda").value;
  var busq = busquedas.find(function(b) { return b.id === busqId; });
  var fuente = document.getElementById("sel-fuente").value;
  var texto = document.getElementById("txt-perfil").value;
  ...
  notionCreate(PROC_DB, props).then(function() {
...
function generarOutreach() {
  var nombre = document.getElementById("c-nombre").value.trim();
  var perfil = document.getElementById("c-perfil").value.trim();
  var busqueda = document.getElementById("c-busqueda").value.trim();
  if (!nombre || !perfil || !busqueda) return;
  document.getElementById("btn-outreach").disabled = true;
  document.getElementById("outreach-step").style.display = "inline";
  document.getElementById("resultado-outreach").innerHTML = "";
  callClaude(OUTREACH_SYS, "Candidato: " + nombre + "\\nPerfil: " + perfil + "\\nB\u00fasqueda: " + busqueda).then(function(raw) {
    var r = JSON.parse(raw.replace(/\`\`\`json|\`\`\`/g, "").trim());
    document.getElementById("resultado-outreach").innerHTML = "<div class='card'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;'><div class='card-title' style='margin-bottom:0;'>Email</div><button class='btn btn-sm btn-ghost' onclick='copiarTexto(\"email-body\",this)'>Copiar</button></div><div style='font-size:12px;color:var(--text3);margin-bottom:8px;'>Asunto: <strong>" + r.asunto + "</strong></div><div id='email-body' class='copy-block'>" + r.mensaje + "</div></div><div class='card'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;'><div class='card-title' style='margin-bottom:0;'>LinkedIn</div><button class='btn btn-sm btn-ghost' onclick='copiarTexto(\"li-body\",this)'>Copiar</button></div><div id='li-body' class='copy-block'>" + r.mensajeLinkedIn + "</div></div>";
  }).catch(function(e) { console.error(e); }).finally(function() {
    document.getElementById("btn-outreach").disabled = false;
    document.getElementById("outreach-step").style.display = "none";
  });
}
...
function cargarSeguimiento() {
  var btn = document.getElementById("btn-seg");
  btn.disabled = true; btn.textContent = "Cargando...";
  notionQ(PROC_DB, { property: "Estado", select: { equals: "Contactado" } }, [{ property: "Fecha de Contacto", direction: "ascending" }]).then(function(d) {
    var items = (d.results || []).map(function(p) {
      var props = p.properties || {};
      return {
        id: p.id,
        nombre: props.Candidato && props.Candidato.title && props.Candidato.title[0] ? props.Candidato.title[0].plain_text : "Sin nombre",
        busqueda: props["B\u00fasqueda Asociada"] && props["B\u00fasqueda Asociada"].rich_text && props["B\u00fasqueda Asociada"].rich_text[0] ? props["B\u00fasqueda Asociada"].rich_text[0].plain_text : "",
        fecha: props["Fecha de Contacto"] && props["Fecha de Contacto"].date ? props["Fecha de Contacto"].date.start : null
      };
    });
    var lista = document.getElementById("lista-seguimiento");
    if (!items.length) {
      lista.innerHTML = "<div style='text-align:center;color:var(--text3);font-size:13px;padding:40px;'>No hay candidatos contactados</div>";
    } else {
      lista.innerHTML = items.map(function(p) {
        var dias = p.fecha ? Math.floor((Date.now() - new Date(p.fecha)) / 86400000) : null;
        var urgente = dias !== null && dias > 5;
        return "<div class='seg-card' style='border-left:3px solid " + (urgente ? "#DC2626" : "#52B788") + ";'><div style='display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;'><div><div style='font-weight:500;font-size:14px;'>" + p.nombre + "</div><div style='font-size:12px;color:var(--text3);margin-top:2px;'>" + p.busqueda + "</div></div>" + (dias !== null ? "<span class='badge " + (urgente ? "badge-red" : "badge-blue") + "'>" + dias + " d\u00edas sin respuesta</span>" : "") + "</div><div id='fu-" + p.id + "'><button class='btn btn-sm btn-ghost' onclick='genFollowup(\"" + p.id + "\",\"" + p.nombre.replace(/"/g, "") + "\",\"" + p.busqueda.replace(/"/g, "") + "\"," + (dias || 0) + ")'>Generar follow-up</button></div></div>";
      }).join("");
    }
  }).catch(function() {
    document.getElementById("lista-seguimiento").innerHTML = "<div style='color:#DC2626;padding:16px;'>Error cargando</div>";
  }).finally(function() {
    btn.disabled = false; btn.textContent = "Actualizar";
  });
}

function genFollowup(id, nombre, busqueda, dias) {
  var div = document.getElementById("fu-" + id);
  div.innerHTML = "<span style='font-size:12px;color:var(--text3);'>Generando...</span>";
  callClaude("Agente de recruiting. Gener\u00e1s follow-ups breves y directos. Solo el texto, sin JSON.", "Candidato: " + nombre + ". B\u00fasqueda: " + busqueda + ". D\u00edas sin respuesta: " + dias + ".").then(function(txt) {
    div.innerHTML = "<div class='result-box'>" + txt + "</div>";
  }).catch(function() {
    div.innerHTML = "<span style='color:#DC2626;font-size:12px;'>Error generando</span>";
  });
}
...
<div><div class="logo-text">V&iacute;nculo</div><div class="logo-sub">Sistema de Agentes</div></div>
</div>
...
<div class="row2">
<div><label class="lbl">B&uacute;squeda activa *</label><select id="sel-busqueda" onchange="onBusquedaChange()"><option value="">Cargando...</option></select></div>
...
<div style="font-size:11px;color:var(--text3);margin-top:3px;">O arrastr&aacute; el archivo ac&aacute;</div>
</div>
<input type="file" id="pdf-input" accept=".pdf" style="display:none" onchange="handlePDF(this)">
<div class="divider">o peg&aacute; el texto manualmente</div>
<div class="field">
<label class="lbl">Texto del perfil</label>
<textarea id="txt-perfil" rows="5" placeholder="Peg&aacute; ac&aacute; el texto del perfil de LinkedIn o CV..."></textarea>
</div>
...
<div class="field"><label class="lbl">Nombre del candidato</label><input id="c-nombre" type="text" placeholder="Ej: Martina Gonz&aacute;lez"/></div>
<div class="field"><label class="lbl">Resumen del perfil</label><textarea id="c-perfil" rows="3" placeholder="Ej: Gerente de Marketing, 10 a&ntilde;os..."></textarea></div>
<div class="field"><label class="lbl">Descripci&oacute;n de la b&uacute;squeda</label><textarea id="c-busqueda" rows="3" placeholder="Ej: Gerente Comercial para empresa industrial..."></textarea></div>
...
<div id="lista-seguimiento"><div style="text-align:center;color:var(--text3);font-size:13px;padding:40px;">Presion&aacute; Actualizar para cargar</div></div>
</div>
</div>
