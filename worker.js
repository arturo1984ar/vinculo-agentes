...
let busquedas=[];
function setTab(t,el){document.querySelectorAll(".panel").forEach(p=>p.classList.remove("active"));document.querySelectorAll(".tab").forEach(b=>b.classList.remove("active"));document.getElementById("panel-"+t).classList.add("active");el.classList.add("active");}
...
async function notionQ(dbId,filter,sorts){const body={};if(filter)body.filter=filter;if(sorts)body.sorts=sorts;const r=await fetch("/functions/notion",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:"/v1/databases/"+dbId+"/query",method:"POST",body})});return r.json();}
async function notionCreate(dbId,props){const r=await fetch("/functions/notion",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({path:"/v1/pages",method:"POST",body:{parent:{database_id:dbId},properties:props}})});return r.json();}
...
busquedas=(d.results||[]).map(p=>({id:p.id,puesto:p.properties?.Puesto?.title?.[0]?.plain_text||"Sin título",cliente:p.properties?.Cliente?.rich_text?.[0]?.plain_text||"",perfil:p.properties?.["Perfil Ideal"]?.rich_text?.[0]?.plain_text||"",sector:p.properties?.Sector?.rich_text?.[0]?.plain_text||"",seniority:p.properties?.Seniority?.select?.name||""}));
...
}catch(e){document.getElementById("header-badge").textContent="Error de conexión";document.getElementById("header-badge").className="badge badge-red";}
}
function onBusquedaChange(){const b=busquedas.find(x=>x.id===document.getElementById("sel-busqueda").value);const info=document.getElementById("busq-info");if(b&&(b.seniority||b.sector)){info.style.display="block";info.textContent=[b.puesto,b.seniority,b.sector,b.cliente].filter(Boolean).join(" · ");}else info.style.display="none";}
...
rp.innerHTML=\`<div class="card-title">Perfil extraído</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">\${campos.map(([l,v])=>\`<div><div class="lbl" style="margin-bottom:3px;">\${l}</div><div style="font-size:13px;">\${v}</div></div>\`).join("")}</div>\${c.resumen?\`<div class="result-box">\${c.resumen}</div>\":""}\`;
...
const barColor=v=>v>=75?"#52B788":v>=55?"#FBBF24":"#F87171";
window._c=c;window._s=s;
const rs=document.getElementById("resultado-scorer");
rs.style.display="block";
rs.innerHTML=\`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;"><div class="card-title" style="margin-bottom:0;">Evaluación del agente</div><div style="display:flex;gap:8px;align-items:center;"><span class="badge \${recColor[s.recomendacion]||'badge-gray'}">\${s.recomendacion}</span><span class="score-pill" style="background:\${scoreBg};color:\${scoreFg};">\${s.score}/100</span></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">\${Object.entries(s.dimensiones||{}).map(([k,v])=>\`<div><div style="display:flex;justify-content:space-between;font-size:12px;"><span style="color:var(--text3);">\${dimLabels[k]||k}</span><span style="font-weight:500;">\${v}</span></div><div class="bar-bg"><div class="bar-fill" style="width:\${v}%;background:\${barColor(v)};"></div></div></div>\`).join("")}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;">\${s.fortalezas?.length?\`<div><div style="font-size:11px;font-weight:500;color:#16A34A;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;">Fortalezas</div>\${s.fortalezas.map(f=>\`<div style="font-size:12px;color:var(--text2);margin-bottom:3px;">• \${f}</div>\`).join("")}</div>\`:""}\${s.brechas?.length?\`<div><div style="font-size:11px;font-weight:500;color:#DC2626;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:6px;">Brechas</div>\${s.brechas.map(b=>\`<div style="font-size:12px;color:var(--text2);margin-bottom:3px;">• \${b}</div>\`).join("")}</div>\`:""}</div><div class="result-box" style="margin-bottom:16px;">\${s.justificacion}</div><div style="display:flex;align-items:center;gap:12px;"><button class="btn btn-primary" id="btn-guardar" onclick="guardarNotion()">💾 Guardar en Notion</button><span id="saved-msg" style="display:none;" class="success-msg">✓ Guardado en Procesos</span></div>\`;
setStep("");
...
try{
const props={Candidato:{title:[{text:{content:c.nombre||"Sin nombre"}}]},"Búsqueda Asociada":{rich_text:[{text:{content:busq?.puesto||""}}]},Cliente:{rich_text:[{text:{content:busq?.cliente||""}}]},Score:{number:s.score},"Justificación del Score":{rich_text:[{text:{content:s.justificacion||""}}]},Estado:{select:{name:"Identificado"}},Fuente:{select:{name:fuente}},"Incluir en Reporte":{checkbox:s.score>=70},"Texto Crudo Original":{rich_text:[{text:{content:texto.slice(0,2000)}}]}};
...
function copiarTexto(id,btn){navigator.clipboard?.writeText(document.getElementById(id).innerText);btn.textContent="✓ Copiado";setTimeout(()=>btn.textContent="Copiar",2000);}
...
const items=(d.results||[]).map(p=>({id:p.id,nombre:p.properties?.Candidato?.title?.[0]?.plain_text||"Sin nombre",busqueda:p.properties?.["Búsqueda Asociada"]?.rich_text?.[0]?.plain_text||"",fecha:p.properties?.["Fecha de Contacto"]?.date?.start||null}));
if(!items.length){lista.innerHTML='<div style="text-align:center;color:var(--text3);font-size:13px;padding:40px;">No hay candidatos en estado "Contactado" por ahora</div>';}
else{lista.innerHTML=items.map(p=>{const dias=p.fecha?Math.floor((Date.now()-new Date(p.fecha))/86400000):null;const urgente=dias!==null&&dias>5;return\`<div class="seg-card" style="border-left:3px solid \${urgente?"#DC2626":"#52B788"};"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;"><div><div style="font-weight:500;font-size:14px;">\${p.nombre}</div><div style="font-size:12px;color:var(--text3);margin-top:2px;">\${p.busqueda}</div></div>\${dias!==null?\`<span class="badge \${urgente?"badge-red":"badge-blue"}">\${dias} días sin respuesta</span>\`:""}</div><div id="fu-\${p.id}"><button class="btn btn-sm btn-ghost" onclick="genFollowup('\${p.id}','\${p.nombre.replace(/'/g,"\\\\'")}','\${p.busqueda.replace(/'/g,"\\\\'")}',\${dias||0})">Generar follow-up</button></div></div>\`;}).join("");}
...
try{const txt=await callClaude("Agente de recruiting. Generás follow-ups breves y directos. Solo el texto, sin JSON.",\`Candidato: \${nombre}. Búsqueda: \${busqueda}. Días sin respuesta: \${dias}.\`);div.innerHTML=\`<div class="result-box">\${txt}</div>\`;}
